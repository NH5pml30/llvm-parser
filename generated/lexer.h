...
public:
  ...
  parser_exception(const std::string &message, int line, int col)
      : std::runtime_error(std::to_string(line) + ":" + std::to_string(col) + " " + message),
        line(line),
        col(col)
  {
  }
...
private:
  ...
  std::vector<std::pair<std::regex, bool>> matchers = {
    ...
    {std::regex(R"__del(var)__del"), true},
  ...
  void push_token(int id, bool passed, int length)
  {
    if (passed)
    {
      ...
      cur_token_ = token{9 + id, cur_line_idx, cur_line_pos, cur_line.substr(cur_line_pos, length)};
    }
    else
